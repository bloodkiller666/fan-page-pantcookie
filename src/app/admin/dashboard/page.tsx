...
import { MdBolt, MdDelete, MdDns, MdEdit, MdForum, MdGroups, MdLogout, MdMonitor, MdRefresh, MdSensors, MdShield, MdSportsEsports, MdLogin, MdSync, MdAutoGraph } from 'react-icons/md';
import SocialAnalyticsContent from '../../../components/admin/SocialAnalyticsContent';
...
export default function AdminDashboard() {
    ...
    const [activeTab, setActiveTab] = useState<'scores' | 'messages' | 'analytics' | 'moderation'>('scores');
    ...
    const handleDeleteMessage = async (id: string) => {
        ...
        if (result.success) {
            ...
            alert(t('admin.moderation.errorDelete') || 'Error al eliminar mensaje');
        }
    };
    ...
    const saveEditMessage = async () => {
        ...
        if (result.success) {
            ...
            alert(t('admin.moderation.errorUpdate') || 'Error al actualizar mensaje');
        }
    };
    ...
    const handleDeleteScore = async (id: string) => {
        if (!confirm(t('admin.moderation.confirmDeleteScore') || '¿Estás seguro de eliminar este puntaje?')) return;
        ...
        if (error) alert(t('admin.moderation.errorDelete') || 'Error al eliminar');
        else {
    ...
    const saveEdit = async () => {
        ...
        if (error) {
            alert(t('admin.moderation.errorUpdate') || 'Error al actualizar');
        } else {
                    ...
                    <div className="space-y-6">
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder={t('admin.masterPassword')}
                    ...
                    <div className="flex items-center gap-10">
                        ...
                        <nav className="hidden xl:flex items-center gap-8">
                            ...
                            <button onClick={() => setActiveTab('scores')} className={`flex items-center gap-2 text-[11px] font-bold tracking-widest transition-opacity ${activeTab === 'scores' ? 'text-primary border-b-2 border-primary pb-1' : 'text-zinc-600 dark:text-slate-300 opacity-70 hover:opacity-100'}`}>
                                <MdSportsEsports className="text-[18px]" /> {t('admin.tabs.scores')}
                            </button>
                            <button onClick={() => setActiveTab('messages')} className={`flex items-center gap-2 text-[11px] font-bold tracking-widest transition-opacity ${activeTab === 'messages' ? 'text-primary border-b-2 border-primary pb-1' : 'text-zinc-600 dark:text-slate-300 opacity-70 hover:opacity-100'}`}>
                                <MdForum className="text-[18px]" /> {t('admin.tabs.messages')}
                            </button>
                            <button onClick={() => setActiveTab('analytics')} className={`flex items-center gap-2 text-[11px] font-bold tracking-widest transition-opacity ${activeTab === 'analytics' ? 'text-primary border-b-2 border-primary pb-1' : 'text-zinc-600 dark:text-slate-300 opacity-70 hover:opacity-100'}`}>
                                <FiMonitor className="text-[18px]" /> {t('admin.tabs.analytics')}
                            </button>
                                ...
                                <div className="flex flex-wrap items-center gap-3">
                                    {activeTab === 'scores' && (
                                        <>
                                            ...
                                            >
                                                <option value="all">{t('admin.dashboard.allGames')}</option>
                                    ...
                                    <button onClick={activeTab === 'scores' ? fetchScores : fetchMessages} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary/10 text-primary text-xs font-bold hover:bg-primary/20 transition-all border border-primary/20">
                                        <MdRefresh className="text-sm" /> {t('admin.dashboard.refresh')}
                                    </button>
                                </div>
                            </div>
                        ...
                        <section className="glass-panel backdrop-blur-xl bg-white dark:bg-slate-900/60 rounded-3xl overflow-hidden border border-rose-500/20 shadow-2xl flex flex-col animate-in slide-in-from-bottom duration-500">
                             <div className="p-6 border-b border-rose-500/10 flex items-center justify-between bg-rose-500/5">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h2 className="font-display font-black text-xl tracking-tight uppercase text-zinc-950 dark:text-white flex items-center gap-2">
                                            {t('admin.moderation.centerTitle')}
                                            <span className="text-[10px] bg-rose-500 text-white px-2 py-0.5 rounded-full">FLAGGED</span>
                                        </h2>
                                        <p className="text-[10px] font-bold text-rose-500 uppercase tracking-widest opacity-70">{t('admin.moderation.purgatoryDesc')}</p>
                                    </div>
                                </div>
                                <button onClick={fetchMessages} className="p-2 hover:bg-rose-500/10 rounded-lg text-rose-500 transition-all">
                                    <MdRefresh className="text-xl" />
                                </button>
                            </div>

                            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[600px] overflow-y-auto custom-scrollbar">
                                {messages.filter(m => m.status === 'banned').length === 0 ? (
                                    <div className="col-span-2 py-20 text-center flex flex-col items-center gap-4 opacity-30">
                                        <MdShield className="text-6xl" />
                                        <p className="font-display font-bold uppercase tracking-widest text-sm">{t('admin.moderation.noMessages')}</p>
                                    </div>
                                ) : (
                                    messages.filter(m => m.status === 'banned').map((msg) => (
                                        <div key={msg.id} className="glass-panel p-5 rounded-2xl border border-rose-500/10 bg-rose-500/[0.02] flex flex-col gap-4 group hover:border-rose-500/30 transition-all">
                                            <div className="flex justify-between items-start">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-8 h-8 rounded-full bg-rose-500/20 flex items-center justify-center text-rose-500 font-bold text-xs uppercase">
                                                        {msg.username[0]}
                                                    </div>
                                                    <div>
                                                        <p className="text-xs font-bold text-zinc-950 dark:text-white uppercase">{msg.username}</p>
                                                        <p className="text-[9px] text-rose-500 font-bold uppercase opacity-60">{msg.country} • Flagged by AI</p>
                                                    </div>
                                                </div>
