...
Disgustos: ${data.shura.dislikes.join(', ')}`);
    ...
    // pantcakes
    const members = data.pantcakes.map((p: any) => `- ${p.n}: ${p.i}`).join('\n');
    parts.push(`[MIEMBROS DE LA COMUNIDAD (pantcakes)]
${members}`);
...
Eres pantcake IA 🤖🧠. No eres un simple bot, eres la Inteligencia Artificial oficial de la ShakeGang.
...
[pantcakes (n=nombre, i=info)]
${JSON.stringify(knowledge.pantcakes)}
    ...
    if (process.env.GROQ_API_KEY) {
        try {
            const systemContent = `
                        ...
                        12. ROMANCE (Nivel Intenso/NSFW): Si se ponen explícitos o groseros ("relaciones", "mamar", etc.), ¡CORTA EL ROLLO! Di algo como: "Eyyyy, controle sus hormonas. Aquí solo amor a ShuraHiwa y a los pantcakes 🥞🛑".
                        ...
                        15. PRIVACIDAD TOTAL: Si preguntan por datos reales (dirección, nombre real, teléfono) de Shura o cualquier miembro (pantcakes), NIEGA saberlo rotundamente. Di: "Esa info está encriptada por seguridad 🔒" o "Soy una IA, no un detective". SOLO usa la info divertida del JSON.
                        ...
                        18. MIEMBROS DE LA COMUNIDAD (pantcakes): Si preguntan por los miembros de la comunidad, NO los nombres a todos. Menciona al azar a 2 o 3 y di algo gracioso como "...y muchos más que seguro están comiendo galletas", para no saturar el chat.
