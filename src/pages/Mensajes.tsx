...
const MensajesContent = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const adminApiKey = process.env.NEXT_PUBLIC_ADMIN_PASSWORD || 'admin123';
      ...
      let status: 'approved' | 'banned' = 'approved';

      if (moderationResponse.ok) {
        const moderationResult = await moderationResponse.json();
        status = moderationResult.status === 'banned' ? 'banned' : 'approved';
      } else {
    ...
    <div ref={containerRef} className="min-h-screen pt-20 pb-10 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {/* READ MODE — Vertical Grid */}
          {activeTab === 'read' && (
            <div className="relative w-full py-10 mb-20 px-4">
              {messages.length === 0 ? (
                <div className="text-center py-20 flex flex-col items-center text-gray-400 w-full bg-white dark:bg-black/20 rounded-3xl border-2 border-dashed border-gray-200 dark:border-white/5">
                  <FaImage size={64} className="mb-4 opacity-50" />
                  <p className="text-2xl italic">{t('wall.emptyWall')}</p>
                  <p className="mt-2 text-sm">{t('wall.emptySub')}</p>
                  <button onClick={() => handleTabChange('write')} className="mt-8 poke-button-pink text-white font-bold px-8 py-3 rounded-xl transition-all">
                    {t('wall.emptyGoWrite')}
                  </button>
                </div>
              ) : (
                <div className="flex flex-col gap-10">
                  {/* BG Title (Static) */}
                  <div className="absolute inset-0 flex items-start justify-center pointer-events-none z-0 mt-20 opacity-20 overflow-hidden">
                    <h2 className="text-[20vw] font-black text-slate-200 dark:text-zinc-800 uppercase italic select-none leading-none tracking-tighter -rotate-12">
                      POKÉWALL
                    </h2>
                  </div>

                  {/* Counter bar */}
                  <div className="relative z-20 bg-[var(--poke-pink)] dark:bg-gray-800/80 backdrop-blur-md rounded-2xl border border-[var(--poke-pink)]/20 py-4 px-6 md:px-10 flex items-center justify-between mb-8 shadow-xl">
                    <div className="flex items-center gap-3">
                      <div className="w-3 h-3 rounded-full bg-white animate-pulse" />
                      <p className="font-pixel text-[10px] md:text-[12px] text-white tracking-[0.2em] uppercase">
                        Sistema de Registros Activo
                      </p>
                    </div>
                    <p className="font-pixel text-[10px] md:text-[12px] text-white/80 tracking-[0.1em] uppercase">
                      {messages.length} ENTRENADORES
                    </p>
                  </div>
                      ...
                      >
                        <div className="flex justify-between items-start mb-6">
                          <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-2xl bg-[var(--poke-pink)]/10 dark:bg-white/10 flex items-center justify-center border border-[var(--poke-pink)]/20 shadow-sm">
                              <FaUser className="text-[var(--poke-pink)] text-xl" />
                            </div>
                            <div>
                              <h4 className="text-gray-900 dark:text-white font-bold text-lg leading-none mb-1.5">{msg.username}</h4>
                              <div className="flex items-center gap-2">
                                <span className="font-pixel text-[8px] text-gray-400 uppercase tracking-widest">{msg.country}</span>
                        ...
                        <div className="mt-auto flex justify-between items-center font-pixel text-[8px] text-gray-400/80 border-t border-gray-100 dark:border-white/5 pt-4">
