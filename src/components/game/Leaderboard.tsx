...
const Leaderboard = ({ difficulty, category, currentPlayer, game = 'puzzle' }: LeaderboardProps) => {
    ...
    return (
        <aside className="w-full lg:w-80 shrink-0 h-full animate-fade-in">
            <div className="bg-white/5 dark:bg-slate-900/40 backdrop-blur-xl rounded-[2rem] h-full flex flex-col overflow-hidden border border-primary/20 shadow-2xl relative group/sidebar">
                {/* Header */}
                <div className="p-6 border-b border-primary/20 flex items-center justify-between bg-primary/5">
                    <h2 className="font-black text-xl flex items-center gap-3 italic tracking-tighter text-slate-100">
                    ...
                    {loading ? (
                                ...
                                <div
                                    ...
                                    className={`flex items-center gap-4 p-4 rounded-2xl transition-all duration-500 border group/item
                                        ${isCurrent 
                                            ...
                                            : 'bg-white/5 dark:bg-slate-800/30 border-white/5 hover:border-primary/30 hover:bg-primary/5 hover:translate-x-1'
                                        }
                                    `}
                                >
                                    {/* Rank Number */}
                                    <div className={`flex items-center justify-center w-9 h-9 rounded-xl font-black text-xs shrink-0 shadow-lg transform group-hover/item:scale-110 transition-transform
                                        ${rank === 1 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600 text-background-dark shadow-yellow-500/20' : 
                                          rank === 2 ? 'bg-gradient-to-br from-slate-300 to-slate-500 text-background-dark shadow-slate-400/20' :
                                          rank === 3 ? 'bg-gradient-to-br from-amber-600 to-amber-800 text-white shadow-amber-700/20' :
                                          'bg-background-dark/50 text-slate-500 border border-primary/20'}
                                    `}>
                                        {rank}
                                    </div>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-1.5">
                                            <p className={`text-sm font-black truncate tracking-tight transition-colors
                                                ${isCurrent ? 'text-primary' : 'text-slate-100 group-hover/item:text-primary'}
                                            `}>
                                                {score.playerName}
                                            </p>
                                            {isCurrent && (
                                        ...
                                        <p className="text-[9px] text-slate-500 uppercase font-black tracking-widest mt-0.5 opacity-70 group-hover/item:opacity-100 transition-opacity">
                                    ...
                                    <div className="text-right">
                                        <p className={`text-sm font-black italic tracking-tighter
                                            ${isCurrent || rank === 1 ? 'text-primary drop-shadow-[0_0_8px_rgba(13,185,242,0.3)]' : 'text-slate-300 group-hover/item:text-primary'}
                                        `}>
                                            {game === 'puzzle' 
                        ...
                        <div className="flex flex-col items-center justify-center h-64 text-center px-6">
                            ...
                            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">
                                {t('common.noScores') || 'Sin registros aún'}
                            </p>
                ...
                <div className="p-6 bg-primary/5 border-t border-primary/10">
                    <button className="w-full py-3 bg-white/5 hover:bg-primary/10 border border-primary/20 rounded-xl text-[10px] font-black text-primary uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 group-hover/sidebar:gap-3 group/btn">
                        {t('common.viewFullRanking') || 'Ver Ranking Completo'}
